...
describe('OrderTicket (下单表单)', () => {
  ...
  it('given_insufficient_funds_when_submit_buy_then_shows_mapped_error', async () => {
    const mockFetch = vi.fn((url: string, opts?: RequestInit) => {
      if (url.includes('/trading/accounts/acc-1/buy') && opts?.method === 'POST') {
  ...
  it('given_insufficient_position_when_submit_sell_then_shows_mapped_error', async () => {
    const mockFetch = vi.fn((url: string, opts?: RequestInit) => {
      if (url.includes('/trading/accounts/acc-1/sell') && opts?.method === 'POST') {
        return Promise.resolve(mockError('INSUFFICIENT_POSITION', '可用持仓不足'))
      }
      throw new Error(`unexpected: ${url}`)
    })
    vi.stubGlobal('fetch', mockFetch)
...
describe('/trading/analytics (分析报表)', () => {
  ...
  it('given_risk_assessment_pending_when_render_then_shows_pending_banner', async () => {
    const mockFetch = vi.fn((url: string, opts?: RequestInit) => {
      if (url.endsWith('/users/me')) return Promise.resolve(mockEnvelope(meData))
      if (url.endsWith('/trading/accounts') && (!opts || !opts.method || opts.method === 'GET')) {
      ...
      if (url.includes('/equity-curve')) return Promise.resolve(mockEnvelope([]))
      ...
      if (url.includes('/risk-assessment') && (!opts || !opts.method || opts.method === 'GET')) {
        return Promise.resolve(
          mockError('RISK_ASSESSMENT_PENDING', '评估正在生成中', 202),
        )
      }
      // risk-assessment evaluate POST 也返回 PENDING
      if (url.includes('/risk-assessment/evaluate') && opts?.method === 'POST') {
        return Promise.resolve(
          mockError('RISK_ASSESSMENT_PENDING', '评估正在生成中', 202),
        )
      }
