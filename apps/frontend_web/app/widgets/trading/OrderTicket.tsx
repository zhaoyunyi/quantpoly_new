...
export function OrderTicket({ accountId, onSuccess }: OrderTicketProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!symbol.trim() || Number.isNaN(qty) || qty <= 0 || Number.isNaN(prc) || prc <= 0) {
    ...
    <div className="bg-bg-card rounded-md shadow-card border border-secondary-300/20 p-lg">
      ...
      <div className="flex gap-xs mb-md">
        <button
          type="button"
          onClick={() => setSide('BUY')}
          className={`flex-1 py-sm rounded-md text-body font-medium transition-all duration-[120ms] ease-out ${
            side === 'BUY'
