...
def test_admin_delete_user_action_is_audited_and_masked():
    ...
    client.post(
        "/auth/register",
        json={"email": "admin-delete-audit@example.com", "password": "StrongPass123!"},
    )
    client.post(
        "/auth/verify-email",
        json={"email": "admin-delete-audit@example.com"},
    )

    admin = repo.get_by_email("admin-delete-audit@example.com")
    assert admin is not None
    admin.set_role("admin")
    admin.set_level(2)
    repo.save(admin)
    ...
    client.post(
        "/auth/register",
        json={"email": "target-delete-audit@example.com", "password": "StrongPass123!"},
    )
    client.post(
        "/auth/verify-email",
        json={"email": "target-delete-audit@example.com"},
    )
    target = repo.get_by_email("target-delete-audit@example.com")
    assert target is not None
    ...
    assert records
    last = records[-1]
    assert last.action == "users.delete"
    assert last.actor == admin.id
    context_text = str(last.context)
    assert admin_token not in context_text
