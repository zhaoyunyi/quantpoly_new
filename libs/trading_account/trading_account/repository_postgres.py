...
class PostgresTradingAccountRepository:
    """基于 SQLAlchemy Engine 的交易账户仓储。"""
    ...
    def delete_trade(self, *, trade_id: str) -> None:
        with self._engine.begin() as conn:
            self._execute(conn, "DELETE FROM trading_account_trade WHERE id = ?", (trade_id,))
    ...
    def delete_cash_flow(self, *, cash_flow_id: str) -> None:
        with self._engine.begin() as conn:
            self._execute(conn, "DELETE FROM trading_account_cash_flow WHERE id = ?", (cash_flow_id,))
        ...
        *,
        ...
        with self._engine.begin() as conn:
            for symbol, price in price_updates.items():
                ...
                cursor = self._execute(conn, sql, tuple(params))
